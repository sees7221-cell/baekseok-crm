...
function todayText() {
@@ -36,20 +25,20 @@ function monthStartText() {
...
function header(title, sub = "") {
  ...
  `;
@@ -61,106 +50,62 @@ async function showDashboard() {
  setActive("dashboard");
  ...
  mainArea.innerHTML = `
    ...
    <div class="card-grid">
      ...
      <div class="card"><h3>미지급 페이백</h3><div class="number" id="paybackTotal">0원</div></div>
    </div>
...
async function loadDashboard() {
  ...
  const plan = (customers || []).filter(c => c.plan_change_date === today);
  const addon = (customers || []).filter(c => c.addon_end_date === today);
  const birth = (customers || []).filter(c => c.birth_date && c.birth_date.slice(5) === monthDay);
  ...
  document.getElementById("planCount").innerText = plan.length;
  document.getElementById("addonCount").innerText = addon.length;
  document.getElementById("birthCount").innerText = birth.length;
  ...
  html += plan.length ? plan.map(c => taskRow(c, "요금제변경")).join("") : `<p class="empty">대상 없음</p>`;

  html += `<br><h3>부가서비스 해지 대상</h3>`;
  html += addonTargets.length
    ? addonTargets.map(c => taskRow(c, "부가해지")).join("")
    : `<p class="empty">대상 없음</p>`;
  html += addon.length ? addon.map(c => taskRow(c, "부가해지")).join("") : `<p class="empty">대상 없음</p>`;
  ...
  html += birth.length ? birth.map(c => taskRow(c, "생일")).join("") : `<p class="empty">대상 없음</p>`;
...
async function loadDashboardExtra(monthStart) {
  ...
  const { data: acc } = await supabaseClient.from("accessories").select("*").gte("sale_date", monthStart);
  document.getElementById("accTotal").innerText = won((acc || []).reduce((s, x) => s + Number(x.amount || 0), 0));
  ...
  document.getElementById("paybackTotal").innerText = won((pay || []).filter(x => x.status === "미지급").reduce((s, x) => s + Number(x.amount || 0), 0));
}

function taskRow(c, type) {
@@ -170,16 +115,12 @@ function taskRow(c, type) {
...
@@ -189,11 +130,10 @@ function showCustomers() {
  setActive("customers");
...
@@ -203,7 +143,6 @@ function showCustomers() {
...
@@ -221,7 +160,7 @@ function showCustomers() {
}
...
@@ -233,34 +172,27 @@ async function saveCustomer() {
...
@@ -269,12 +201,12 @@ async function loadCustomers() {
...
@@ -295,40 +227,35 @@ async function loadCustomers() {
async function showCustomerDetail(phone) {
  ...
  const { data: customer, error } = await supabaseClient
    ...
    .eq("phone", phone)
    .single();
  const { data: c, error } = await supabaseClient.from("customers").select("*").eq("phone", phone).single();
...
@@ -344,8 +271,7 @@ async function showCustomerDetail(phone) {
    ...
    <div class="card">
@@ -354,38 +280,27 @@ async function showCustomerDetail(phone) {
    </div>
  `;
...
async function showCustomerEdit(phone) {
  ...
  const { data: c } = await supabaseClient.from("customers").select("*").eq("phone", phone).single();
...
@@ -395,7 +310,7 @@ async function showCustomerEdit(phone) {
}
...
@@ -405,10 +320,7 @@ async function updateCustomer(phone) {
  ...
  const { error } = await supabaseClient
    ...
    .update(updateData)
    .eq("phone", phone);
  const { error } = await supabaseClient.from("customers").update(item).eq("phone", phone);

  if (error) {
    alert("수정 실패: " + error.message);
@@ -419,24 +331,38 @@ async function updateCustomer(phone) {
...
async function deleteCustomer(phone) {
  ...
  const { error } = await supabaseClient.from("customers").delete().eq("phone", phone);
...
async function saveCustomerLog(phone) {
  ...
  const { error } = await supabaseClient
    .from("customer_logs")
    .insert([log]);
  const { error } = await supabaseClient.from("customer_logs").insert([item]);

  if (error) {
    alert("이력 저장 실패: " + error.message);
@@ -448,35 +374,22 @@ async function saveCustomerLog(phone) {
}
...
async function loadCustomerLogs(phone) {
  const { data, error } = await supabaseClient
    ...
    .order("id", { ascending: false });
  const { data } = await supabaseClient.from("customer_logs").select("*").eq("customer_phone", phone).order("id", { ascending: false });
...
@@ -487,11 +400,10 @@ async function showUsedPhones() {
  setActive("used");
...
@@ -505,7 +417,6 @@ async function showUsedPhones() {
...
@@ -537,9 +448,7 @@ async function saveUsedPhone() {
    return;
  }
  ...
  const { error } = await supabaseClient
    .from("used_phones")
    .insert([item]);
  const { error } = await supabaseClient.from("used_phones").insert([item]);

  if (error) {
    alert("저장 실패: " + error.message);
@@ -551,37 +460,27 @@ async function saveUsedPhone() {
}
...
async function loadUsedPhones() {
  ...
  const { data } = await supabaseClient.from("used_phones").select("*").order("id", { ascending: false });
  ...
  list.innerHTML = data.map(x => {
    ...
    `;
@@ -594,25 +493,23 @@ async function showAccessories() {
  ...
  `;
@@ -634,9 +531,7 @@ async function saveAccessory() {
    return;
  }
  ...
  const { error } = await supabaseClient
    .from("accessories")
    .insert([item]);
  const { error } = await supabaseClient.from("accessories").insert([item]);

  if (error) {
    alert("저장 실패: " + error.message);
@@ -648,110 +543,52 @@ async function saveAccessory() {
}
...
async function loadAccessories() {
  ...
  const { data } = await supabaseClient.from("accessories").select("*").order("id", { ascending: false });
  ...
  list.innerHTML = `
    ...
    ${data.map(x => `
      <div class="list-row">
        ...
        <div class="row-meta">판매일: ${x.sale_date || "-"} / 담당자: ${safe(x.manager)}</div>
      </div>
    `).join("")}
  `;
}
...
@@ -782,9 +619,7 @@ async function savePayback() {
    return;
  }
  ...
  const { error } = await supabaseClient
    .from("paybacks")
    .insert([item]);
  const { error } = await supabaseClient.from("paybacks").insert([item]);

  if (error) {
    alert("저장 실패: " + error.message);
@@ -796,82 +631,50 @@ async function savePayback() {
}
...
async function loadPaybacks() {
  ...
  const { data } = await supabaseClient.from("paybacks").select("*").order("id", { ascending: false });
  ...
  const unpaidTotal = data
    ...
    .reduce((sum, item) => sum + Number(item.amount || 0), 0);
  const unpaid = data.filter(x => x.status === "미지급").reduce((s, x) => s + Number(x.amount || 0), 0);
